				...
				if (SUCCEEDED(hr))
				{
					...
					if (SUCCEEDED(hr))
					{
						...
						for (int i = nNbItems - 1; i >= 0; i--)
						{
							...
							menuItemInfo.fMask = MIIM_FTYPE | MIIM_STRING | MIIM_ID | MIIM_SUBMENU | MIIM_DATA | MIIM_STATE;
							...
							if (GetMenuItemInfo(hMenu, i, TRUE, &menuItemInfo))
							{
								if (CompareMenuStrings(pszName, wszPinLocalizedVerb) == 0)
								{
									...
									cmInvokeCommandInfoEx.cbSize = sizeof(cmInvokeCommandInfoEx);
									cmInvokeCommandInfoEx.fMask = 0x00004000;
									cmInvokeCommandInfoEx.hwnd = GetDesktopWindow();
									cmInvokeCommandInfoEx.lpVerb = MAKEINTRESOURCEA(menuItemInfo.wID - 1);
									cmInvokeCommandInfoEx.lpVerbW = MAKEINTRESOURCEW(menuItemInfo.wID - 1);
									cmInvokeCommandInfoEx.nShow = SW_SHOWNORMAL;
									hr = pContextMenu->InvokeCommand((LPCMINVOKECOMMANDINFO)&cmInvokeCommandInfoEx);
									*nPinned = 1;
									break;
								}
								// 应用已被固定到任务栏
