...
namespace PowerTools.Views.Pages
{
    ...
    {
        private readonly string AllString = ResourceService.ShellMenuResource.GetString("All");
        private readonly string EditMenuString = ResourceService.ShellMenuResource.GetString("EditMenu");
        private readonly string IconFilterConditionString = ResourceService.ShellMenuResource.GetString("IconFilterCondition");
        private readonly string NameRegexString = ResourceService.ShellMenuResource.GetString("NameRegex");
        private readonly string NameString = ResourceService.ShellMenuResource.GetString("Name");
        private readonly string NoneString = ResourceService.ShellMenuResource.GetString("None");
        private readonly string ProgramFilterConditionString = ResourceService.ShellMenuResource.GetString("ProgramFilterCondition");
        private readonly string SelectIconString = ResourceService.ShellMenuResource.GetString("SelectIcon");
        private readonly string SelectProgramString = ResourceService.ShellMenuResource.GetString("SelectProgram");
        ...
        protected override async void OnNavigatedTo(NavigationEventArgs args)
        {
            ...
            if (!isInitialized)
            {
                ...
                await Task.Run(() =>
                {
                    RegistryHelper.MonitorRegistryValueChange(Registry.CurrentUser, @"Software\PowerTools\ShellMenu");
                });
            }
        ...
        private async void OnAddMenuItemClicked(object sender, RoutedEventArgs args)
        {
            ...
            await Task.Run(() =>
            {
                ...
                {
                    menuKey = Path.Combine(@"Software\PowerTools\ShellMenu", menuGuid.ToString());
                    menuIndex = 0;
                }
        ...
        {
            if (key.Equals(@"Software\PowerTools\ShellMenu"))
            {
                if (!isChanger)
                {
        ...
        #endregion 第三部分：自定义扩展菜单页面——自定义事件
